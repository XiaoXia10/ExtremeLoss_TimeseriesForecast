...
def plot_loss_curve(log_dir):
    ...
    ax.plot(d.valid_loss.min())
   
    print(d.valid_loss.idxmin())

def make_results_table(path_):
    # return pd.DataFrame({os.path.basename(c): summary(c) for c in glob('logs/*')}).T.sort_values('valid_loss')
    pd.DataFrame({os.path.basename(c): summary(c) for c in glob(path_+"/*")}).T.sort_values('valid_loss')

#%%
# # load results

# path = r"G:\My Drive\Neuchatel_Project\Milandre\python_codes\improved_graph_wavenet\experiment"
# tr_val = loss_curve(path)
