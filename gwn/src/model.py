...
class GWNet(nn.Module):
    def __init__(self, device, num_nodes, dropout=0.3, supports=None, do_graph_conv=True,
                 addaptadj=True, aptinit=None, in_dim=2, out_dim=3,
        ...
        ######################### The problem is here! ########################
        #######################################################################
        # Changed: The Gated TCN is a 1D temporal convolutional module to Gated TCN to 2D and it worked
    ...
    def forward(self, x):
        ...
        # x = F.tanh(skip)  # ignore last X?
        # x = F.tanh(self.end_conv_1(x))
        x = self.end_conv_2(x)  # downsample to (bs, seq_length, n-nodes, n-features)
        return x
