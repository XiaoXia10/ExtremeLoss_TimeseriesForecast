# -*- coding: utf-8 -*-
"""
Created on Wed May 29 12:56:03 2024
...
def main(args):
    ...
    checkpoint_path = join(f"{args.data_path}/{args.save_dir}", args.save_model_name)
    ...
    test_x = np.load(join(f"{args.data_path}/{args.data_dir}", "x_test.npy"))
    test_y = np.load(join(f"{args.data_path}/{args.data_dir}", "y_test.npy"))
    ...
    train_y.to_csv(join(f"{args.data_path}/{args.save_dir}", "train_realy.csv"), index =False)
    val_y.to_csv(join(f"{args.data_path}/{args.save_dir}", "val_realy.csv"), index =False)
    test_y.to_csv(join(f"{args.data_path}/{args.save_dir}", "test_realy.csv"), index =False)
    
    pred_train.to_csv(join(f"{args.data_path}/{args.save_dir}", "train_predy.csv"), index =False)
    pred_val.to_csv(join(f"{args.data_path}/{args.save_dir}", "val_predy.csv"), index =False)
    pred_test.to_csv(join(f"{args.data_path}/{args.save_dir}", "test_predy.csv"), index =False)
